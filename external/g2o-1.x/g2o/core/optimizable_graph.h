...
namespace g2o {
  ...
  struct OptimizableGraph : public HyperGraph {
    ...
    struct Parameters : public HyperGraph::HyperGraphElement
    {
      virtual ~Parameters() {};
      //! read the data from a stream
      virtual bool read(std::istream& is) = 0;
      ...
      inline void setId(int id_) {_id = id_;}
    protected:
      int _id;
    };
    ...
    class VertexCache: public HyperGraph::HyperGraphElement {
    public:
      VertexCache(Vertex* v, int cacheId); 
      virtual void update();
      inline OptimizableGraph::Vertex* vertex() {return _vertex;}
      inline const OptimizableGraph::Vertex* vertex() const {return _vertex;}
      inline int id() const {return _cacheId;}
    protected:
      Vertex* _vertex;
      int _cacheId;
    };

    typedef std::vector<VertexCache*> VertexCacheVector;
	...
	VertexCache* getCache(int cacheId);
	const VertexCache* getCache(int cacheId) const;

	bool setCache(VertexCache* cache);
	
	virtual void updateCache();

	VertexCacheVector& cacheVector() { return _cacheVector; }
	const VertexCacheVector& cacheVector() const { return _cacheVector; }
      protected:
	...
	VertexCacheVector _cacheVector;
    };
	...
	virtual bool setCacheId(int vertexNum, int cacheId);
	virtual int getCacheId(int vertexNum);
	virtual VertexCache* createCache(int vertexNum, OptimizableGraph* g); 
    ...
    virtual bool saveParameters(std::ostream& os) const;
    ...
    G2O_ATTRIBUTE_DEPRECATED(bool setParameters(Parameters* p)) {
      return addParameters(p);
    }

    bool addParameters(Parameters* p) {
      std::map<int, Parameters*>::iterator it=_parametersMap.find(p->id());
      if (it!=_parametersMap.end()) return false;
      _parametersMap.insert(std::make_pair(p->id(), p));
      return true;
    }

    Parameters* parameters(int id) {
      std::map<int, Parameters*>::iterator it=_parametersMap.find(id);
      if (it!=_parametersMap.end()) return it->second;
      return 0;
    }
  ...
  protected:
    ...
    std::map<int, Parameters*> _parametersMap;
  };
