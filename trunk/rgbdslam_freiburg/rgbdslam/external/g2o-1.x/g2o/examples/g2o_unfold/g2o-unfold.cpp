...
void gnudump_edges(string gnudump, 
  ...
  if (gnudump.size() > 0) {
    ...
    for (HyperGraph::EdgeSet::const_iterator eit = begin; eit != end; ++eit) {
      ...
      const VertexSE2* from = dynamic_cast<const VertexSE2*>((*eit)->vertices()[0]);
      const VertexSE2* to   = dynamic_cast<const VertexSE2*>((*eit)->vertices()[1]);
	...
	if (from && to) {
      ...
      if(dumpFeatures) {
	const VertexPointXY* tox  = dynamic_cast<const VertexPointXY*>((*eit)->vertices()[1]);
	if (from && tox) {
	  Vector3d p1 = from->estimate().toVector();
...
int main(int argc, char** argv)
{
  ...
  if (gnudump.size() > 0) 
	...
	const VertexPointXY* tox  = dynamic_cast<const VertexPointXY*>(e->vertices()[1]);
	if (from && tox) {
	...
	OptimizableGraph::Vertex* vfrom = static_cast<OptimizableGraph::Vertex*>(e->vertices()[0]);
	vfrom->fixed() = false;
	OptimizableGraph::Vertex* vto = static_cast<OptimizableGraph::Vertex*>(e->vertices()[1]);
	vto->fixed() = false;
      }
    }
    cerr << "done" << endl;
    ...
    for (EdgeSetPairVector::iterator it = myEdgeStack.begin(); 
	 it != myEdgeStack.end(); ++it) {
      ...
      OptimizableGraph::Vertex* vfrom = static_cast<OptimizableGraph::Vertex*>((*eit)->vertices()[0]);
      OptimizableGraph::Vertex* vto = static_cast<OptimizableGraph::Vertex*>((*eit)->vertices()[1]);
	...
	OptimizableGraph::Vertex* v = dynamic_cast<OptimizableGraph::Vertex*>((*eit)->vertices()[1]);
	v->fixed() = false;
  ...
  //     for (HyperGraph::EdgeSet::const_iterator it = optimizer.edges().begin(); it != optimizer.edges().end(); ++it) {
  //       const VertexSE2* from = dynamic_cast<const VertexSE2*>((*it)->vertices()[0]);
  //       const VertexPointXY* to = dynamic_cast<const VertexPointXY*>((*it)->vertices()[1]);
  //       if (from && to) {
  //         hasLandmarks = true;
  //         break;
  //       }
