#ifndef __G2O_EDGE_CREATOR__
#define __G2O_EDGE_CREATOR__
...
struct EdgeCreator{
  struct EdgeCreatorEntry{  
    EdgeCreatorEntry(const std::string& edgeTypeName, const std::vector<int>& edgeCacheIds)
    :_edgeTypeName(edgeTypeName), _edgeCacheIds(edgeCacheIds) {}

    EdgeCreatorEntry(const std::string& edgeTypeName)
    :_edgeTypeName(edgeTypeName){}

    std::string _edgeTypeName;
    std::vector<int> _edgeCacheIds;
  };
  ...
  //! The order matters.
  //! @param edgeType: the tag of edge to create
  //! @param cacheIds: the ids of the vertex caches to create
  //! @returns false on failure (incompatible types). Currently returns always true because i did not have time to implement checks
  bool addAssociation(const std::string& vertexTypes, const std::string& edgeType, const std::vector<int>& cacheIds);
